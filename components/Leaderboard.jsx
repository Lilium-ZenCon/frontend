		...
		<div className="bg-darkgreen w-full min-h-screen px-10" id="leaderboard">
			<div className="pt-20 text-white w-[40%]">
				...
				<p className="text-md mt-5">
					Welcome to our Carbon Credits Leaderboard! This leaderboard showcases projects that have excelled in generating carbon credits and contributing to the fight against climate change.
					These projects play a vital role in reducing greenhouse gas emissions and promoting sustainability on a global scale.
				</p>
			</div>
			<div className="flex gap-5 justify-between w-full mt-8">
				{leaderboardCards.slice(0, 3).map((card, index) => {
					return <LeaderboardCard key={index} place={index + 1} company={card.company.name} tokens={card.tokens.toString()} />;
				})}
			</div>
			<div className="py-10">
